...
class AIHistory: ObservableObject {
    ...
    func sortSearch(text: String){
        ...
        if query == "profi" || query == "profil" || query.contains("profile") {
            DispatchQueue.main.async {
                self.searchMessages = self.searchMessages.sorted { (message1, message2) in
                    return message1.isProfileBuild && !message2.isProfileBuild
                }
            }
        } else {
            DispatchQueue.main.async {
    ...
    func getChats() async {
        ...
        all.forEach { element in
            if let id = element.id, let text1 = element.text1, let text2 = element.text2, let date = element.date, !text2.isEmpty {
                new.append(iterateMessages(id: "\(UUID())", parentID: id, question: text1, answer: text2, date: date, hasImage: element.hasImage, isProfileBuild: element.isProfileBuild))
                
                if text1.isEmpty || element.hasImage {
    ...
    func processElements(new: [iterateMessages]) async -> [AllMessages] {
        ...
        for element in new {
            if let x = newAll.firstIndex(where: { $0.id == element.parentID }) {
                ...
                let newMesRow = MessageRow(isInteracting: false, isProfileBuild: element.isProfileBuild, send: MessageRowType.rawText(element.question), response: .attributed(attributedSend))
            ...
            mess.forEach { element in
                if let response = element.response?.text, !response.isEmpty {
                    if !self.searchMessages.contains(where: { $0.question == element.send.text && $0.answer == response }) {
                        ...
                        self.searchMessages.insert(iterateMessages(id: "\(UUID())", parentID: parent, question: element.send.text, answer: response, date: Date(), hasImage: hasImage, isProfileBuild: element.isProfileBuild), at: 0)
                        toAdd.append(AIStorage(id: parent, text1: element.send.text, text2: response, date: Date(), hasImage: hasImage, isProfileBuild: element.isProfileBuild))
                    }
                }
            }
            self.getDates()
    ...
    func ModelCreate(allAI: [AIStorage]) {
        for storage in allAI {
            ...
            entity.isProfileBuild = storage.isProfileBuild
        }
...
class ViewModel: ObservableObject {
    ...
    @Published var textOptions = [AIText]()
    @Published var currentPosition: Int = 10000
    var exportedToServer = [String]()
    var cancelledJobs = [String]()
    var isVccBuilding = false
    var appendVccToExisting: String? = nil
    ...
    func sendNormalMessage(command: String, text: String, completion: @escaping(Bool) -> Void) {
        let messages = [MessageAI(id: UUID().uuidString, role: .user, content: command + text, createAt: Date())]
        service.sendNormalMessage(messages: messages) { result in
            switch result {
            case .success(let response):
                if !response.isEmpty {
                    if let index = self.textOptions.firstIndex(where: { $0.oldText.lowercased() == text.lowercased() }) {
                        self.textOptions[index].options.append(AIRec(text: response))
                    } else {
                        self.textOptions.append(AIText(oldText: text, options: [AIRec(text: response)]))
                        self.currentPosition = self.textOptions.count - 1
                    }
                    completion(true)
                } else {
                    completion(false)
                        ...
                        DispatchQueue.main.async {
                            ...
                            Task {
                                ...
                                if let currentOutput = currentOutput, !currentOutput.results.isEmpty {
                                    ...
                                    let tempResult = ParserResult(attributedString: lastAttrString, isCodeBlock: lastResult.isCodeBlock, codeBlockLanguage: lastResult.codeBlockLanguage)
                                    
                                    let idx = results.count - 1
                                    if idx < results.count && idx >= 0 {
                                        results[idx] = tempResult
                                    }
                                ...
                                if idx < self.messages.count && idx >= 0 {
